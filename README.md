...
- **输入区独占一行，控件排在下方** —— 下方一行左侧是工作目录 + 模型 + 思考等级，右侧是圆形发送按钮（生成中才出现停止按钮）；会话建立后目录锁定为只读（pi 把会话绑定在启动目录上）
...
│   ├── agnes.ts                 # Agnes 生图/生视频（凭证 0600，服务端代发）
...
65 条路由（34 GET / 31 POST），全部在 `/api/pi/*` 下，仅监听 `127.0.0.1`。
